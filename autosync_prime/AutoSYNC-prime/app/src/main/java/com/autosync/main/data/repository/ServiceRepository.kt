...
class ServiceRepositoryImpl @Inject constructor(
    ...
    private val vehicleDao: com.autosync.main.data.local.dao.VehicleDao, // Injected to update mileage
    ...
    override suspend fun insertService(service: Service, imageUri: android.net.Uri?) {
        ...
        try {
            ...
            // Update Vehicle Mileage Logic
            if (service.serviceMileage > 0) {
                try {
                    val vehicle = vehicleDao.getVehicleById(service.vehicleId)
                    if (vehicle != null && service.serviceMileage > vehicle.currentMileage) {
                        val updatedVehicle = vehicle.copy(currentMileage = service.serviceMileage)
                        // Update Local
                        vehicleDao.updateVehicle(updatedVehicle)
                        // Update Remote
                        firestore.collection("vehicles").document(vehicle.id.toString()).set(updatedVehicle)
                    }
                } catch (e: Exception) {
                    // Fail silently for mileage update, service is already saved
                    e.printStackTrace()
                }
            }
